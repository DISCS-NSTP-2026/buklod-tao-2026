...
function normalizeLegacyDoc(data) {
  ...
  if (!normalized.household_address) {
    ...
    if (houseNum || street) {
      normalized.household_address = [houseNum, street]
        .filter(Boolean)
        .join(", ");
    ...
    await getDocs(colRef)
      .then(async (querySnapshot) => {
        ...
        querySnapshot.forEach((snapshot) => {
          const { normalized, legacyKeys, fieldUpdates } = normalizeLegacyDoc(
            snapshot.data(),
          );
  ...
  for (var data in partner) {
    ...
    if (partner[data] instanceof Object) {
      editForm.getElementById(data.toString()).value =
        partner[data]._lat + " " + partner[data]._long;
    } else {
      if (partner[data] != null) {
        editForm.getElementById(data.toString()).value =
          partner[data].toString();
      } else {
...
export function submitEditForm() {
  ...
  if (validate_errors.length > 0) {
    ...
    const original_household_name = document.getElementById(
      "original_household_name",
    ).value;
...
export async function submitAddForm() {
  ...
  for (let i = 0; i < BUKLOD_RULES[2].length; i++) {
    ...
    if (
      fieldName == "number_residents" ||
      fieldName == "number_minors" ||
      fieldName == "number_pregnant" ||
      fieldName == "number_pwd" ||
      fieldName == "number_sick" ||
      fieldName == "number_seniors" ||
      fieldName == "number_families" ||
      fieldName == "number_healthy" ||
      fieldName == "exit_points" ||
      fieldName == "knowledge_readiness"
    ) {
